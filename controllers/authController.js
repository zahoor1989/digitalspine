    ...
    //pass in request data here to create user from user schema 
        try {
            const newUser = await User.create({
                ...
                passwordConfirm: req.body.passwordConfirm? req.body.passwordConfirm: req.body.confirmPassword 
            });
...
exports.loginUser = catchAsync(async(req, res, next) => {
    ...
    if (!user || !(await user.correctPassword(password, user.password))) {
        return res.status(401).json(new AppError('Incorrect username or password', 401));
    }
